...
--
--  Version $Revision: 1.8 $ from $Date: 2000/02/28 05:53:38 $
--
...
--      reservedid  -> `as' | `call' | `context' | `enum'
--		     | `fun' | `get' | `header' | `lib' | `prefix' | `set' 
...
--    the tool, as iilegal identifier will never occur in a C header file that
