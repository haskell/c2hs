...
--
dumpCHS :: String -> CHSModule -> Bool -> [String] -> CST s ()
dumpCHS fname mod' pureHaskell extraHsDeps =
  do
    let (suffix, kind) = if pureHaskell
                         then (hssuffix , "(Haskell)")
                         else (chssuffix, "(C->HS binding)")
        baseHsOut = showCHSModule mod' pureHaskell
        hsOut = if not pureHaskell || null extraHsDeps
                then baseHsOut
                else addImports fname baseHsOut extraHsDeps
        contents version' =
          "-- GENERATED by " ++ version' ++ " " ++ kind ++ "\n\
          \-- Edit the ORIGNAL .chs file instead!\n\n"
          ++ hsOut
    CIO.writeFile (fname <.> suffix) (contents version)

-- TODO: 1. find suitable start line for imports
...
makeImportDecl f ln m =
  ImportDecl { importAnn = SrcSpanInfo sp_all [sp_import, sp_qualified, sp_as]
             , importModule = ModuleName (noInfoSpan sp_modname) m
             , importQualified = True
             , importSrc = False
             , importSafe = False
             , importPkg = Nothing
             , importAs = Just $ ModuleName (noInfoSpan sp_c2hsimp) "C2HSImp"
             , importSpecs = Nothing }
  where mlen = length m
        sp_import = SrcSpan f ln 1 ln 7
        sp_qualified = SrcSpan f ln 8 ln 17
        sp_modname = SrcSpan f ln 18 ln (mlen+18)
        sp_as = SrcSpan f ln (mlen+19) ln (mlen+21)
        sp_c2hsimp = SrcSpan f ln (mlen+22) ln (mlen+29)
        sp_all = SrcSpan f ln 1 ln (mlen+29)
